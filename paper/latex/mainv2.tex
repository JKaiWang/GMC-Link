...
\begin{abstract}
Referring multi-object tracking takes a video and a natural-language expression as input and tracks all objects described by the expression. Many expressions describe how an object moves rather than how it appears, yet existing host models often infer motion from raw bounding-box displacement. Under a moving camera, this signal is unreliable: a parked vehicle may appear to move across the image, while a vehicle moving with the camera may show little image-plane displacement and be incorrectly treated as static. Existing motion-language RMOT methods align motion with text but do not explicitly remove camera-induced motion, whereas ego-motion compensation is typically used only for data association. We bridge these two directions with GMC-Link, a decision-level plug-in module that composes frame-to-frame homographies into a cumulative transform, extracts residual motion at multiple temporal gaps, aligns the resulting descriptor with the language expression using a two-tower encoder, and adds the motion score to the host prediction without retraining the host model. On iKUN, GMC-Link improves moving-class HOTA from $20.25$ to $29.08$, corresponding to a gain of $8.83$ points over five random seeds. Most of this improvement comes from introducing motion-language alignment, while ego-motion compensation provides a further gain of $1.77$ moving-class HOTA ($p{=}0.006$) and $2.07$ static-class HOTA ($p{<}10^{-4}$) over raw image-plane motion. These results show that compensation helps both recover truly moving objects and reject stationary objects affected by camera motion. Across the three evaluated host--split settings, larger gains are observed for hosts with lower native moving-class HOTA. In pooled HOTA, GMC-Link matches the published iKUN result and improves FlexHook on Refer-KITTI V2 by $0.281$, while FlexHook V1 remains below its published score because of a known reproduction gap.
\end{abstract}
...
\draw[ar] (text)--(align);
\end{tikzpicture}
\caption{\textbf{GMC-Link pipeline.} ORB homographies compose into a cumulative transform, produce a 13D residual-velocity descriptor, align with the expression, and correct the host score through additive fusion.}
\label{fig:pipeline}
\end{figure}
...
This stage estimates camera motion from consecutive video frames so that object motion can be separated from camera-induced motion. Given the input video frames and tracked object bounding boxes, we extract and match background keypoints using Oriented FAST and Rotated BRIEF (ORB)~\cite{orb}. Keypoints inside tracked object bounding boxes are removed before matching so that moving foreground objects do not affect camera-motion estimation. The remaining matched keypoints are used with Random Sample Consensus (RANSAC)~\cite{ransac} to estimate a homography $H_{t-1\to t}$, which describes the geometric transformation caused by camera motion from frame $I_{t-1}$ to frame $I_t$.
...
To capture motion over different time spans, residual velocities are computed at temporal gaps of $\{2,5,10\}$ frames. The residual velocities are concatenated with object box information to form a 13-dimensional motion descriptor,
\begin{equation}
...
Since GMC-Link only modifies the final prediction score, it can be integrated into existing two-stage RMOT frameworks without changing or retraining their detector, tracker, visual encoder, or language encoder.
...
Results are averaged over three random seeds, while ablation studies use five seeds together with Welch's t-test. The motion-language aligner is trained using AdamW with a learning rate of $10^{-3}$, batch size $256$, and $100$ epochs. Camera motion is estimated using ORB with $1500$ keypoints and RANSAC with a $5$-pixel threshold. Both host models use their original published detection results.

Refer-KITTI does not provide a dedicated validation split. Therefore, the fusion parameters are calibrated on the benchmark sequences. To assess whether this calibration overfits the evaluation data, we additionally perform leave-one-sequence-out (LOSO) calibration. In each run, the fusion parameters are selected using all but one sequence and evaluated on the held-out sequence. Across all evaluated settings, the LOSO results remain consistently above the native host, indicating that the observed improvements are not simply due to overfitting the calibration sequences.
...
After applying the proposed GMC-Link module, performance improves on all settings compared with the uncalibrated baseline. On iKUN, GMC-Link achieves $44.634 \pm 0.066$, which is comparable to the published result ($44.564$) while introducing additional motion reasoning. On FlexHook V2, GMC-Link improves pooled HOTA from $42.526$ to $42.807 \pm 0.038$, demonstrating that the proposed module can improve overall tracking performance. Although FlexHook V1 does not exceed its published score, it consistently outperforms the uncalibrated baseline, indicating that the calibrated motion score is more effective than directly using raw motion information.

Overall, these results show that camera-motion-compensated motion information is beneficial only when it is properly aligned with the host model. More importantly, GMC-Link can improve motion reasoning while maintaining the original tracking performance of the host model, making it suitable as a plug-in module for existing RMOT frameworks.
...
In future work, we plan to investigate more robust camera motion estimation methods and richer motion representations that can better handle complex dynamic scenes. We also hope to evaluate GMC-Link on a broader range of referring multi-object tracking frameworks to further improve its generalization.
