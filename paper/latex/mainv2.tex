...
Referring multi-object tracking (RMOT) outputs trajectories for the objects described by a free-form language. For example, a language description like \emph{left-vehicles-in-red} mentioned in the Refer-KITTI dataset is used to evaluate the performance of RMOT. Many such descriptions focus on the movement of targeted objects, such as ``moving cars'' and ``a vehicle turning left'', rather than their appearance. However, the motion of objects in a video captured from a moving camera is mixed with the object's motion and the camera motion. A static car may appear to move in video frames due to camera motion, while a nearby moving car may maintain a nearly fixed distance from the moving camera and appear to be static.

VMRMOT~\cite{vmrmot} and its RMOT lineage~\cite{transrmot,ikun,flexhook} align motion with text. But the motion, mixed with object and camera motion, is not specially distinguished. Plain MOT proposes to consider motion compensation. BoT-SORT~\cite{botsort} and UCMCTrack~\cite{ucmctrack} are adopted to estimate camera motion and stabilize predictions, respectively. However, the motion information and language description are not jointly processed or fused. The motion of the camera or vehicle can be obtained from the global positioning system (GPS) and inertial measurement unit (IMU) annotations provided by KITTI or estimated from image-based methods such as ground-plane projection. Existing RMOT methods, however, do not incorporate this information into language-guided tracking. Instead, our method estimates this motion from image features, removing the need of GPS or IMU sensors. This design also allows the module to be directly integrated into different two-stage RMOT frameworks without modifying their architecture. The only additional requirement is a single scalar calibration for each host model, as described in Section~\ref{sec:fusion}.

Global Motion Compensation-Link (GMC-Link) separates camera motion from object motion to provide motion information for language-guided tracking. It consists of four stages. First, a homography, which models the geometric transformation between consecutive frames, is estimated to describe camera motion. The estimated homographies are accumulated across multiple frames to compensate for camera motion. Second, residual velocity, which represents object motion after camera motion has been removed, is computed over multiple frames and used as a temporal motion descriptor. Third, the Contrastive Motion Aligner measures how well the motion descriptor matches the language expression. Finally, the resulting motion score is combined with the host model through a calibrated decision-level correction, allowing GMC-Link to improve existing two-stage RMOT frameworks without modifying their architecture.

The accumulated camera transformation is the key component of GMC-Link. By removing camera motion, it allows the motion descriptor to represent the object's movement rather than the camera's movement. It also preserves motion information across multiple frames, enabling the model to capture motion patterns described by expressions such as ``moving'' or ``turning''. Based on this design, we hypothesize that camera motion compensation is essential for distinguishing object motion from camera-induced motion. If residual velocity is replaced with raw image velocity, performance should decrease on moving objects while stationary objects are more likely to be misclassified as moving. Section~\ref{sec:exp} evaluates this hypothesis and shows that residual velocity contributes most of the improvement for moving objects, while camera motion compensation makes the motion descriptor effective for object disambiguation rather than simply providing additional motion cues.
...
\begin{itemize}
  ...
  \item We provide experimental evidence showing that camera motion compensation is the key factor for improving motion-aware referring multi-object tracking.
...
GMC-Link consistently improves motion-aware tracking on iKUN and FlexHook while preserving the original detector, tracker, and host architecture.
