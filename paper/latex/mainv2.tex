...
The first stage estimates camera motion between consecutive frames to separate camera motion from object motion. We use Oriented FAST and Rotated BRIEF (ORB)~\cite{orb}, a feature detector and descriptor, to extract and match background keypoints between adjacent frames. To prevent moving objects from affecting camera motion estimation, keypoints inside the tracked object bounding boxes are removed before matching. The remaining correspondences are used to estimate a homography $H_{t-1\to t}$ using Random Sample Consensus (RANSAC)~\cite{ransac}. The estimated homography $H_{t-1\to t}$ represents the geometric transformation caused by camera motion from frame $I_{t-1}$ to frame $I_t$.
...
\label{eq:cumulative}
\end{equation}
where $H_{t-k\to t}$ represents the accumulated camera transformation from frame $I_{t-k}$ to frame $I_t$. Instead of repeatedly transforming already warped coordinates, the accumulated homography is applied once to the original object centroid. This strategy reduces the accumulation of numerical errors while providing a stable reference for measuring object motion.

The accumulated homography compensates for camera motion before motion features are extracted. The compensated object trajectories are then passed to the next stage, where residual velocity is computed to describe the object motion after removing the effect of camera motion.
...
The motion matching score is combined with the prediction of the host model at the decision level. The final score is computed as
\begin{equation}
