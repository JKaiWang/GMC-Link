...
%DIF DEL gmc_v1.tex   Sat Aug 22 18:05:41 2026
%DIF ADD gmc_v2.tex   Wed Aug 26 09:08:37 2026
...
\begin{itemize}
  ...
  \item Across iKUN and FlexHook, the GMC Module improves pooled HOTA in all three evaluated host settings. On iKUN, it raises MOVING HOTA from $25.531$ to $32.606\pm0.654$ (approximately $+7.08$ points).
\end{itemize}
...
\label{sec:ego}
This stage estimates camera motion from consecutive video frames so that object motion can be separated from ego-motion. Refer-KITTI is recorded from a driver-view camera, so the road surface is visible in most frames and stays close to a single plane. A homography is exact for a plane, which makes the road a reliable reference for camera motion. We select Shi-Tomasi corners~\cite{shitomasi} in the lower half of the frame, excluding the \DIFdelbegin \DIFdel{region where }\DIFdelend \DIFaddbegin \DIFadd{interiors of the previous frame's }\DIFaddend tracked object boxes\DIFdelbegin \DIFdel{overlap}\DIFdelend , and follow them into the next frame with pyramidal Lucas-Kanade optical flow~\cite{lucaskanade}. RANSAC~\cite{ransac} then estimates the frame-to-frame homography $H_{t-1 \to t}$ from these correspondences. \DIFdelbegin \DIFdel{Sparse road texture can make this fit fail; the module then falls back to a global homography estimated from ORB~\mbox{%DIFAUXCMD
\cite{orb} }\hskip0pt%DIFAUXCMD
features over the whole frame, with object boxes masked out}\DIFdelend \DIFaddbegin \DIFadd{For a forward-facing camera the horizon lies near the vertical image center: pixels above it belong to buildings and sky, which are not on the road plane, whereas the lower half contains the road from directly ahead of the vehicle to the horizon. We therefore fit the homography to the lower half only}\DIFaddend .
...
\label{sec:residual}
Using the accumulated homography matrix and the objects' bounding boxes, this stage estimates the residual velocity, which represents object motion after removing the estimated camera motion. For a temporal gap $g$, the raw velocity \DIFdelbegin \DIFdel{$v^{\mathrm{raw}}_{g}$ is derived }\DIFdelend \DIFaddbegin \DIFadd{$v^{\mathrm{raw}}_{g} = (\boldsymbol{o}_{t}-\boldsymbol{o}_{t-g})/g$ is the observed image-plane velocity of an object, computed }\DIFaddend from the displacement of \DIFdelbegin \DIFdel{the object centroid between two frames (}\DIFdelend \DIFaddbegin \DIFadd{its centroid $\boldsymbol{o}$ between frames }\DIFaddend $I_{t-g}$ and $I_{t}$\DIFdelbegin \DIFdel{) and is therefore mixed with objectmotion and camera motion }\DIFdelend \DIFaddbegin \DIFadd{; it is the sum of the object's own motion and the motion induced by the camera}\DIFaddend . To remove the camera motion, the accumulated homography \DIFaddbegin \DIFadd{$H_{t-g \to t}$ }\DIFaddend is applied to \DIFdelbegin \DIFdel{an object's }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend centroid $\boldsymbol{o}_{t-g}$ \DIFdelbegin \DIFdel{at $I_{t-g}$}\DIFdelend \DIFaddbegin \DIFadd{in homogeneous coordinates}\DIFaddend , yielding $\hat{\boldsymbol{o}}_{t}$, the centroid predicted at $I_{t}$ under the assumption that the object is stationary. The ego \DIFdelbegin \DIFdel{displacement $\hat{\boldsymbol{o}}_{t}-\boldsymbol{o}_{t-g}$ thus arises from }\DIFdelend \DIFaddbegin \DIFadd{velocity $v^{\mathrm{ego}}_{g} = (\hat{\boldsymbol{o}}_{t}-\boldsymbol{o}_{t-g})/g$ is thus the image-plane velocity induced by }\DIFaddend camera motion alone, \DIFdelbegin \DIFdel{and dividing it by the temporal gap $g$ yields the ego velocity $v^{\mathrm{ego}}_{g}$}\DIFdelend \DIFaddbegin \DIFadd{evaluated at the object's location}\DIFaddend . The residual velocity $v^{\mathrm{res}}_{g}$ is \DIFdelbegin \DIFdel{then computed as
}\DIFdelend \DIFaddbegin \DIFadd{the camera-motion-compensated object velocity, obtained by subtracting the ego velocity from the raw velocity:
}\DIFaddend \begin{equation}
  v^{\mathrm{res}}_{g} = v^{\mathrm{raw}}_{g} - v^{\mathrm{ego}}_{g} \DIFdelbegin \DIFdel{,
  }\DIFdelend \DIFaddbegin \DIFadd{= }\bigl(\boldsymbol{o}\DIFadd{_{t}-}\hat{\boldsymbol{o}}\DIFadd{_{t}}\bigr)\DIFadd{/g.
  }\DIFaddend \label{eq:residual}
\end{equation}
This representation approximates the motion that a stationary camera would observe. For example, a parked vehicle has near-zero residual velocity even when camera motion causes a large image displacement. All velocities are normalized by the image dimensions to make them independent of image resolution. \DIFaddbegin \DIFadd{In the implementation the factor $1/g$ is omitted; this is equivalent up to a per-gap rescaling of the first projection layer of Sec.~\ref{sec:align}.
}\DIFaddend 
...
\DIFaddend \subsection{Motion-language alignment}
\label{sec:align}
...
The proposed module is attached to two host models, iKUN~\cite{ikun} and FlexHook~\cite{flexhook}. For comparison, we also evaluate a raw-velocity variant that uses image-plane motion directly, without ego-motion compensation. The GMC Module is trained using AdamW~\cite{adamw} with a learning rate of $10^{-3}$, batch size 256, and 100 epochs. The \DIFaddbegin \DIFadd{aligner is trained on the Refer-KITTI V1 training split (15 sequences, disjoint from the test sequences). Each positive pair consists of an expression and the motion feature of a ground-truth object annotated as its referent, computed from the object's ground-truth boxes; tracker outputs are used only at test time. The }\DIFaddend road-plane homography tracks up to 600 corners with a 3-pixel RANSAC threshold \DIFdelbegin \DIFdel{, while the global homography uses 1,500 ORB keypoints with a 5-pixel threshold. Across the four evaluation sequences, the road-plane homography }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend succeeds on all \DIFdelbegin \DIFdel{2,065 }\DIFdelend \DIFaddbegin \DIFadd{7,690 }\DIFaddend adjacent frame pairs \DIFdelbegin \DIFdel{, so the global homography is never used as a fallback}\DIFdelend \DIFaddbegin \DIFadd{of the 19 training and evaluation sequences}\DIFaddend . Excluding host inference, the module runs at \DIFdelbegin \DIFdel{$31.8$ }\DIFdelend \DIFaddbegin \DIFadd{$149$ }\DIFaddend FPS on CPU \DIFaddbegin \DIFadd{($6.7$\,ms per frame)}\DIFaddend .
...
Table~\ref{tab:deficit} reports HOTA variations for movement-related expressions. All three host settings improve, and the size of the improvement differs by an order of magnitude: iKUN gains $+7.08\pm0.65$, FlexHook V1 $+0.67\pm0.13$, and FlexHook V2 $+0.18\pm0.06$.
...
\begin{table}
  ...
  \begin{tabular}{lcc}
    ...
    Full module                & \textbf{32.30 $\pm$ 0.63} & \textbf{44.80 $\pm$ 0.10} \\
    $-$ego (raw velocity)      & 28.49 $\pm$ 0.35          & 44.17 $\pm$ 0.07          \\
    $-$multiscale (single gap) & 30.45 $\pm$ 0.20          & 44.49 $\pm$ 0.02          \\
    \hline
  \end{tabular}
\end{table}
