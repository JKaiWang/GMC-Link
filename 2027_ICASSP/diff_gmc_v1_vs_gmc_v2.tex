...
%DIF DEL gmc_v1.tex   Tue Aug 25 01:26:18 2026
%DIF ADD gmc_v2.tex   Tue Aug 25 03:03:28 2026
...
The proposed module is attached to two host models, iKUN~\cite{ikun} and FlexHook~\cite{flexhook}. For comparison, we also evaluate a raw-velocity variant that uses image-plane motion directly, without ego-motion compensation. The GMC Module is trained using AdamW~\cite{adamw} with a learning rate of $10^{-3}$, batch size 256, and 100 epochs. The road-plane homography tracks up to 600 corners with a 3-pixel RANSAC threshold \DIFdelbegin \DIFdel{, while the global homography uses 1,500 ORB keypoints with a 5-pixel threshold. Across the four evaluation sequences, the road-plane homography }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend succeeds on all 2,065 adjacent frame pairs \DIFdelbegin \DIFdel{, so the global homography is never used as a fallback}\DIFdelend \DIFaddbegin \DIFadd{across the four evaluation sequences}\DIFaddend . Excluding host inference, the module runs at $31.8$ FPS on CPU.
