...
\begin{table}
  \centering
  \caption{Pooled HOTA on the Refer-KITTI test sets (mean $\pm$ std over three seeds) and the change in DetA\,/\,AssA relative to the reproduced host.}
  \label{tab:main}
  \footnotesize
...
\begin{table}
  \centering
  \caption{Per-class HOTA ablation on iKUN ($n{=}5$); single $\alpha$: $\alpha_{\mathrm{mot}}{=}\alpha_{\mathrm{app}}{=}0.35$ (LOSO).}
  \label{tab:ablation}
  \footnotesize
  \setlength{\tabcolsep}{2pt}
  \begin{tabular}{lccc}
