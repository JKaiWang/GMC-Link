...
\node[box] (frames) {\textcolor{red}{Driver-view frames $I_{t-1},\,I_t$}};
\node[box, below=of frames] (orb) {\textcolor{red}{Road-region ORB correspondences\\ BFMatcher + RANSAC $\Rightarrow H_{t-1\to t}$}};
\node[box, below=of orb] (cum) {Compose cumulative $H_{t-k\to t}$;\\ warp original centroids};
\node[box, below=of cum] (desc) {\textcolor{red}{12D residual velocity}\\ gaps $\{2,5,10\}$ + box state};
\node[box, below=of desc] (align) {Motion-language alignment\\ $\Rightarrow$ cosine score};
\node[box, below=of align] (fuse) {\textcolor{red}{Expression-aware fusion\\ $s_{\text{host}}+\alpha s_{\text{gmc}}$}};
...
\draw[ar] (text)--(align);
\end{tikzpicture}
\caption{\textbf{GMC-Link pipeline.} \textcolor{red}{ORB homographies estimated from road-region features} compose into a cumulative transform, produce a 12D residual-motion feature, align with the expression, and correct the host score through expression-aware late fusion.}
\label{fig:pipeline}
\end{figure}

\subsection{Ego-motion compensation}
\label{sec:ego}
\textcolor{red}{This stage estimates camera motion from consecutive video frames so that object motion can be separated from camera-induced motion. Because Refer-KITTI videos are captured from a driver-view camera in driving scenes, background correspondences around the road region provide a natural geometric reference for camera-motion estimation. We extract ORB~\cite{orb} keypoints from consecutive frames and retain road-region background features, excluding points inside tracked foreground bounding boxes where required. The remaining features at the $(t-1)$-th frame $I_{t-1}$ and the $t$-th frame $I_t$ are matched using the existing matching procedure, and RANSAC~\cite{ransac} estimates the frame-to-frame homography $H_{t-1 \to t}$. Applying the composed transformation to the original object centroid then yields the camera-induced displacement used to derive residual motion.}
...
\begin{equation}
\textcolor{red}{s_{\mathrm{final}} = s_{\mathrm{host}} + \alpha\,s_{\mathrm{gmc}}},
\label{eq:fusion}
\end{equation}
where $s_{\mathrm{host}}$ is the prediction score produced by the host model. \textcolor{red}{The fusion weight $\alpha$ is selected according to the motion semantics of the referring expression.}
...
FlexHook V1~\cite{flexhook}  & \textcolor{red}{53.824} & \textcolor{red}{54.011 $\pm$ 0.025} & \textcolor{red}{$+0.187$} \\
FlexHook V2~\cite{flexhook}  & \textcolor{red}{42.526} & \textcolor{red}{42.658 $\pm$ 0.030} & \textcolor{red}{$+0.132$} \\
\hline
\end{tabular}
\end{table}


Table~\ref{tab:deficit} reports HOTA variations for movement-related expressions. \textcolor{red}{The largest improvement is observed for iKUN ($+7.08\pm0.65$), while FlexHook V1 improves by $+0.49\pm0.21$ and FlexHook V2 remains nearly unchanged ($+0.01\pm0.03$).}
...
\textcolor{red}{FlexHook V1} & \textcolor{red}{44.31} & \textcolor{red}{$+0.49\pm0.21$} \\
\textcolor{red}{FlexHook V2} & \textcolor{red}{48.01} & \textcolor{red}{$+0.01\pm0.03$} \\
\hline
\end{tabular}
\end{table}
...
\label{sec:ablation}
\textcolor{red}{Table~\ref{tab:ablation} reports the five-seed ablation measurements on iKUN. The full module improves MOVING HOTA from $25.53$ to $32.30\pm0.63$ and STATIC HOTA from $44.22$ to $44.80\pm0.10$. Removing ego-motion compensation reduces MOVING and STATIC HOTA to $28.49$ and $44.17$, respectively, while removing the multi-scale gaps gives $30.45$ and $44.49$. These results support the contribution of both components.}

\begin{table}
\centering
\caption{\textcolor{red}{Per-class HOTA ablation on iKUN.}}
\label{tab:ablation}
\small
\setlength{\tabcolsep}{2pt}
\begin{tabular}{lcc}
...
\textcolor{red}{$-$ego (raw velocity)} & \textcolor{red}{28.49 $\pm$ 0.65} & \textcolor{red}{44.17 $\pm$ 0.12} \\
\textcolor{red}{$-$multiscale (single gap)} & \textcolor{red}{30.45 $\pm$ 0.67} & \textcolor{red}{44.49 $\pm$ 0.11} \\
\hline
\end{tabular}
\end{table}
...
GMC-Link is a decision-level plug-in module that improves motion grounding in referring multi-object tracking without modifying the detector, tracker, or appearance-language matching model. By compensating for camera motion and aligning residual motion with language descriptions, the proposed module helps distinguish object motion from camera-induced motion. \textcolor{red}{Road-region background correspondences provide a geometric reference for estimating camera motion in driver-view driving scenes, after which the residual motion is aligned with language and integrated through expression-aware late fusion. On Refer-KITTI, pooled HOTA improves in all three evaluated host settings. For movement-related expressions, the largest gain is observed on iKUN ($+7.08\pm0.65$), and the ablation results show improvements in both MOVING and STATIC HOTA for the full module.}

Despite these promising results, several limitations remain. First, the proposed camera motion compensation relies on ORB and RANSAC, which estimate a single homography and may become less accurate in scenes with strong parallax or large non-planar motion. Second, \textcolor{red}{expression-aware fusion requires calibration of host-specific weights and deterministic lexical cues}. Finally, our experiments are conducted on two RMOT frameworks and the Refer-KITTI benchmarks, and additional evaluations on more host architectures and datasets are needed to further validate the generality of the proposed approach.
