...
**文中的 A3、A25 這類編號是什麼**:實驗記錄編號。每個編號對應 `RESEARCH_NOTES.md` §10 表格裡的一列,該列記錄那次實驗的完整內容 —— 預登記文件(`docs/PREREG_*.md`)、跑法、原始數據位置(`results/` 下的 JSON)、統計結論。想查本文任何數字的出處:拿編號去 `RESEARCH_NOTES.md` 搜(例如搜「A25」)。
...
| + warmup 遮罩(下述第 1 條) | 44.634 | +0.122 | 30.043 | — |
| + 刪 EMA、similarity ego(第 2 條) | 44.656 | +0.022 | 30.045 | — |
