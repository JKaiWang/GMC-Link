...
| A46 | **TransRMOT third-host campaign — prereg committed (126b2f5, docs/PREREG_TRANSRMOT_HOST_2026_09.md) + advisor side-track S1/S2 done; inference blocked on the NVIDIA driver** (2026-09-02. P0 diagnosis: kernel 7.0.0-30 has no nvidia module — `nvidia-driver-580-open` installed but no dkms and no prebuilt module for this kernel; repair = `apt install dkms linux-headers-$(uname -r) nvidia-dkms-580-open` + modprobe/reboot, user-side. Host facts verified: official checkpoint0099 on disk (`~/RMOT/exps/default/`), native gate threshold = `filter_dt_by_ref_scores(...,0.5)` inference.py:575, predict.txt has NO score column → P2 = score-dumping inference re-run, GPU-only in practice (CPU fallback exists but ~150k frame-forwards). Exp 18's old TransRMOT+GMC (+4.55, min-fusion) is superseded and not comparable. **S2 router audit** (`results/moving_kw/s2_router_audit.json`): independent semantic labels of the 150 official V1 test expressions under the A43 three-class definition vs `gmc_link.moving_kw.classify` → **150/150 agreement** (21/12/117); the only 'misrouting' left is the DEFINITIONAL exclusion of direction/turning/speed exprs (214/818), already disclosed in §5 — the 'router misfires' question is closed. **S1 ego-magnitude stratification** (`results/moving_kw/s1_ego_stratification.json`, local `s1_ego_stratification.py`; ego = mean px displacement of a 16×8 road-band grid under the road homography, averaged over each expression's GT frames; gain = per-expr HOTA ship(am1.0_aa0.1, n=3) − native from TrackEval detailed CSV — NB detailed CSV is 0–1 scale, ×100): mean ego 0005 52.3 px / 0011 22.6 / 0013 24.3. MOVING class (n=21): Spearman ρ=0.41, p=0.065 — positive but MARGINAL, and ego variance is mostly between-sequence while gains at the same ego level span 0…+30 (e.g. 26.2 px: +0.4 vs +30.2). All-150: ρ=0.12 n.s. Verdict: the advisor's strong claim ('gain tracks ego magnitude') is NOT supported at expression level; a mild positive trend only — usable as one cautious sentence, not a figure. The measured explanation of the iKUN-vs-FH gap remains host motion deficit (2026-05-26), which ego magnitude logically cannot explain (same videos). Largest per-expr gains: 0013+left-people/persons-who-are-walking +29/+30, 0013+left-pedestrian +17.5, moving-cars 0011/0005 +13.3/+12.3; zeros are host silent-fails. **CAMPAIGN COMPLETE 2026-09-12** (driver repaired: prebuilt `linux-modules-nvidia-580-open-generic-7.0`, no dkms needed). **P2**: inference re-run with a pre-gate score dump (`scores.txt` per expr, `%.17g` full precision — a first `%.2f` run was discarded, breaks the α=0 bit gate), 150 exprs, `~/RMOT/exps/off150_scores/`; two host-repo compat patches (py3.9 lazy gmc_link import; torch-2.6 `weights_only=False`). **P3 gate**: α=0 reproduces native **150/150 predict.txt byte-identical**; native decomposition 39.09 (old tree/old enum/inference gt) → 36.986 (official-150, list −2.10) → 37.486 (new tree, runtime drift +0.50 — torch/driver upgraded since 2025-10; cancels in every Δ) → **45.757 pooled / 33.883 MOVING** (gt_template/, convention +8.27); anchors: repo README corrected 38.06 ≈ inference-gt band, paper pre-correction 46.56 ≈ gt_template band. **P4**: 9 caches (3 seeds × 3 seqs; entries 32,727/103,531/16,093 identical across seeds); warm-11 applied INLINE per expression — TransRMOT candidate streams are expression-conditioned (track queries see the sentence), so `filter_warmup_cache.py`'s union-grid replay does NOT apply. **P5 LOSO** (10×10 two-α grid × 3 folds × 3 seeds, fold-mean, `_seqs` dirs, `results/transrmot_host/loso_transrmot.json`): fold argmaxes (0.7,0.2)/(1.5,1.0)/(0.5,0.5), no censoring → **(α_mot, α_app) = (0.7, 0.5)**; single-α picks 0.35/1.0/0.5 → **α\*=0.5**. **Full test n=3** (`transrmot_host.json`): two-α **46.149±0.025 pooled / 35.317±0.131 MOVING** (+0.392 t=27 / +1.434 t=19 one-sample df=2), STATIC 44.086±0.103 (+0.41), APPEAR 48.053±0.025 (+0.29); single-α 0.5: 46.139±0.023 / 35.203±0.183 (two-α worth only +0.11 MOVING here — near-degenerate, unlike iKUN's +4.57). **H1 SUPPORTED**: native-MOVING vs MOVING-gain now four settings, monotone: iKUN 27.70→+9.44 · TransRMOT 33.88→+1.43 · FH V2 38.56→+0.69 · FH V1 47.90→+0.43. **H2 REJECTED**: pooled +0.39, no TempRMOT-style regression — track-query propagation is NOT the temporal-memory failure mode; the boundary stays specifically at temporal-memory hosts. **External corroboration (2026-09-13, NOT citable)**: C²RMOT (OpenReview X8fyHZPG6i, anonymous under review) reports TransRMOT at 45.65 / DetA 36.15 / AssA 57.86 on Refer-KITTI V1 — same band as our reproduction 45.757 / 36.64 / 57.40, so our number sits in the modern re-evaluation consensus; kept out of the paper (anonymous submission, unstable reference) | prereg 2026-09-02; P2–P6 measured 2026-09-12 | **third architecture lands ON the host-deficit curve (H1); no temporal regression (H2 rejected); TransRMOT ship = two-α (0.7, 0.5): 46.149±0.025 pooled / MOVING +1.43** |
