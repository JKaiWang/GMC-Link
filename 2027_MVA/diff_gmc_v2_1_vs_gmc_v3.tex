...
%DIF ADD gmc_v3.tex     Sun Sep 13 10:59:47 2026
...
\setlength{\floatsep}{7pt plus 2pt minus 2pt} %DIF > 
\setlength{\textfloatsep}{9pt plus 2pt minus 2pt} %DIF > 
...
\setlength{\belowcaptionskip}{4pt} %DIF > 
 %DIF > 
%DIF -------
% Example definitions.
% --------------------
...
\begin{equation}
  ...
  \label{eq:fusion}
\end{equation}
where $s_{\mathrm{host}}$ is the prediction score from the host model. The weighting $\alpha_c$ is designed to vary for different types of expressions. For example, \enquote{moving car} is a \emph{moving} expression, \enquote{left vehicles which are parking}\footnote{in Refer-KITTI, this expression refers to a vehicle that is already parked, rather than a vehicle that is in the process of parking.} is a \emph{static} expression, and \enquote{right cars in light color} is a\DIFaddbegin \DIFadd{n}\DIFaddend  \emph{appearance} expression. Based on the keywords in the expression, we classify it as moving, static, or appearance. Because different host models produce scores over different ranges, the weighting $\alpha_c$ is selected for each host via leave-one-sequence-out cross-validation. For each fold, the weighting that yields the highest pooled HOTA (on the validation set) is chosen for all but one sequence. The final weighting is determined as the median over folds. Finally, \DIFdelbegin \DIFdel{when iKUN is used as the host model, the weighting $\alpha_c$ is set to 0.7 for moving and static expressions, while the weighting $\alpha_c$ is set to }\DIFdelend \DIFaddbegin \DIFadd{the selected weightings for the four host settings are listed in Table~\ref{tab:alphas}. For both FlexHook settings, the procedure selects the same weighting for all expression classes, so the routing degenerates to a single weighting.
}

\begin{table}[t]
  \centering
  \caption{\DIFaddFL{Fusion weightings $\alpha_c$ selected by leave-one-sequence-out cross-validation.}}
  \label{tab:alphas}
  \footnotesize
  \begin{tabular}{lcc}
    \hline
    \DIFaddFL{Host        }& \DIFaddFL{$\alpha_{\mathrm{mot}}$ (moving/static) }& \DIFaddFL{$\alpha_{\mathrm{app}}$ (appearance) }\\
    \hline
    \DIFaddFL{iKUN        }& \DIFaddFL{1.0 }& \DIFaddendFL 0.1 \DIFdelbeginFL \DIFdelFL{for the appearance expressions.
When }\DIFdelendFL \DIFaddbeginFL \\
    \DIFaddFL{TransRMOT   }& \DIFaddFL{0.7 }& \DIFaddFL{0.5 }\\
    \DIFaddendFL FlexHook V1 \DIFdelbeginFL \DIFdelFL{is used as the host model, the weighting $\alpha_c$ is set to }\DIFdelendFL \DIFaddbeginFL & \DIFaddendFL 7   \DIFdelbeginFL \DIFdelFL{for all expressions. When }\DIFdelendFL \DIFaddbeginFL & \DIFaddFL{7   }\\
    \DIFaddendFL FlexHook V2 \DIFdelbeginFL \DIFdelFL{is used as the host model, the weighting $\alpha_c$ is set to }\DIFdelendFL \DIFaddbeginFL & \DIFaddendFL 5   \DIFdelbeginFL \DIFdelFL{for all expressions. 
}\DIFdelendFL \DIFaddbeginFL & \DIFaddFL{5   }\\
    \hline
  \end{tabular}
\end{table}
\DIFaddend 
...
\DIFdelendFL \DIFaddbeginFL \begin{table}[t]
  ...
  \caption{\DIFdelbeginFL \DIFdelFL{Performance in terms of the pooled }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Pooled }\DIFaddendFL HOTA\DIFaddbeginFL \DIFaddFL{, DetA, and AssA }\DIFaddendFL on the Refer-KITTI test \DIFdelbeginFL \DIFdelFL{dataset}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{sets}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{Each host row gives the baseline (HOTA as published; DetA and AssA from our reproduction); the +\,GMC row gives the fused model (HOTA as mean $\pm$ std over three seeds). The better value of each pair is in bold. $^*$ denotes our reproduction using the official implementation, under the host's own frame convention (Sec.~\ref{sec:setup}).}\DIFaddendFL }
  \label{tab:main}
...
\DIFdelendFL \DIFaddbeginFL \footnotesize
  \DIFaddendFL \begin{tabular}{lccc}
    \hline
    \DIFdelbeginFL \DIFdelFL{Host                        }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Method                      }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{w/o GMC }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{HOTA               }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{w. GMC           }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{DetA  }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{Gain                      }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{AssA  }\DIFaddendFL \\
    \hline
    \DIFaddbeginFL \multicolumn{4}{c}{\textit{\DIFaddFL{Refer-KITTI}}} \\
    \hline
    \DIFaddendFL iKUN~\cite{ikun}            & \DIFdelbeginFL \DIFdelFL{44.564    }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{44.56              }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{44.847 $\pm$ 0.107 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{32.04 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{62.48 }\\
    \DIFaddFL{\quad }\DIFaddendFL +\DIFdelbeginFL \DIFdelFL{0.283 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\,GMC                }& \textbf{\DIFaddFL{45.304 $\pm$ 0.115}} & \textbf{\DIFaddFL{32.90}} & \textbf{\DIFaddFL{62.94}} \DIFaddendFL \\
    \DIFaddbeginFL \hline
    \DIFaddFL{TransRMOT~\mbox{%DIFAUXCMD
\cite{transrmot}  }\hskip0pt%DIFAUXCMD
}& \DIFaddFL{45.757$^*$         }& \DIFaddFL{36.64 }& \DIFaddFL{57.40 }\\
    \DIFaddFL{\quad +\,GMC                }& \textbf{\DIFaddFL{46.149 $\pm$ 0.025}} & \textbf{\DIFaddFL{36.95}} & \textbf{\DIFaddFL{57.89}} \\
    \hline
    \DIFaddendFL FlexHook V1~\cite{flexhook} & 53.824             & \DIFdelbeginFL \DIFdelFL{53.980 $\pm$ 0.059 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{43.35 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{66.92 }\\
    \DIFaddFL{\quad }\DIFaddendFL +\DIFdelbeginFL \DIFdelFL{0.156 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\,GMC                }& \textbf{\DIFaddFL{53.980 $\pm$ 0.059}} & \textbf{\DIFaddFL{43.51}} & \textbf{\DIFaddFL{67.07}} \DIFaddendFL \\
    \DIFaddbeginFL \hline
    \multicolumn{4}{c}{\textit{\DIFaddFL{Refer-KITTI-V2}}} \\
    \hline
    \DIFaddendFL FlexHook V2~\cite{flexhook} & 42.526             & \DIFdelbeginFL \DIFdelFL{42.625 $\pm$ 0.032 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{30.63 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{59.19 }\\
    \DIFaddFL{\quad }\DIFaddendFL +\DIFdelbeginFL \DIFdelFL{0.099 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\,GMC                }& \textbf{\DIFaddFL{42.625 $\pm$ 0.032}} & \textbf{\DIFaddFL{30.75}} & \textbf{\DIFaddFL{59.23}} \DIFaddendFL \\
    \hline
  \end{tabular}
\end{table}

Table~\ref{tab:deficit} reports HOTA variations \DIFdelbegin \DIFdel{for movement-related }\DIFdelend \DIFaddbegin \DIFadd{on the moving-class }\DIFaddend expressions only. As can be seen, the GMC module provides performance improvement across all \DIFdelbegin \DIFdel{three }\DIFdelend \DIFaddbegin \DIFadd{four }\DIFaddend settings. The performance gains differ by an order of magnitude: iKUN gains \DIFdelbegin \DIFdel{$+7.08\pm0.65$, FlexHook V1 $+0.67\pm0.13$}\DIFdelend \DIFaddbegin \DIFadd{$+9.44\pm0.92$, TransRMOT $+1.43\pm0.13$, FlexHook V2 $+0.69\pm0.04$}\DIFaddend , and FlexHook \DIFdelbegin \DIFdel{V2 $+0.18\pm0.06$. }\DIFdelend \DIFaddbegin \DIFadd{V1 $+0.43\pm0.19$. The gain decreases monotonically as the host's baseline moving-class HOTA increases, across three different architectures: the weaker the host's own motion understanding, the more the compensated motion cues contribute.
}\DIFaddend 
...
\DIFdelendFL \DIFaddbeginFL \begin{table}[t]
  \DIFaddendFL \centering
  \caption{Moving-class HOTA \DIFdelbeginFL \DIFdelFL{on movement-related expressions }\DIFdelendFL (\DIFdelbeginFL \DIFdelFL{$27/158$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{moving-class expressions: $21/150$ }\DIFaddendFL for iKUN, \DIFdelbeginFL \DIFdelFL{$25/150$ for }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{TransRMOT, and }\DIFaddendFL FlexHook V1, \DIFdelbeginFL \DIFdelFL{$136/862$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$111/862$ }\DIFaddendFL for FlexHook V2), $n{=}3$ seeds. \DIFaddbeginFL \DIFaddFL{Rows are ordered by baseline moving-class HOTA.}\DIFaddendFL }
  \label{tab:deficit}
  \small
  ...
  \begin{tabular}{lcc}
    ...
    \DIFdelbeginFL \DIFdelFL{FlexHook V1 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{TransRMOT   }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{44.31    }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{33.88    }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$+0.67\pm0.13$   }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$+1.43\pm0.13$   }\DIFaddendFL \\
    ...
    \DIFaddbeginFL \DIFaddFL{FlexHook V1 }& \DIFaddFL{47.90    }& \DIFaddFL{$+0.43\pm0.19$   }\\
    \hline \DIFaddendFL
  \end{tabular}
\end{table}
...
\DIFdelendFL \DIFaddbeginFL \begin{tabular}{lccc}
    ...
    Full module (w. GMC)      & \textbf{\DIFdelbeginFL \DIFdelFL{32.30 $\pm$ 0.63}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{36.99}\DIFaddendFL } & \textbf{\DIFdelbeginFL \DIFdelFL{44.80 $\pm$ 0.10}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{46.05}\DIFaddendFL } \DIFaddbeginFL & \textbf{\DIFaddFL{45.28}} \DIFaddendFL \\
    \DIFaddbeginFL \DIFaddFL{single $\alpha$ (no routing) }& \DIFaddFL{32.42        }& \DIFaddFL{45.99          }& \DIFaddFL{44.95          }\\
    \DIFaddendFL \quad - ego (raw velocity only) & \DIFdelbeginFL \DIFdelFL{28.49 $\pm$ 0.35          }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{32.37     }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{44.17 $\pm$ 0.07          }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{45.77          }& \DIFaddFL{44.61          }\DIFaddendFL \\
    \quad - multiscale (single gap) & \DIFdelbeginFL \DIFdelFL{30.45 $\pm$ 0.20          }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{35.14     }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{44.49 $\pm$ 0.02          }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{45.96          }& \DIFaddFL{45.00          }\DIFaddendFL \\
    \hline
  \end{tabular}
\end{table}
