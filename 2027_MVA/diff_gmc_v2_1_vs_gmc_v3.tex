...
%DIF ADD gmc_v3.tex     Sun Sep 13 12:41:17 2026
...
%DIFDELCMD < %%%
\DIFdelend Table~\ref{tab:deficit} reports HOTA variations \DIFdelbegin \DIFdel{for movement-related }\DIFdelend \DIFaddbegin \DIFadd{on the moving-class }\DIFaddend expressions only. As can be seen, the GMC module provides performance improvement across all \DIFdelbegin \DIFdel{three }\DIFdelend \DIFaddbegin \DIFadd{four }\DIFaddend settings. The performance gains differ by an order of magnitude\DIFdelbegin \DIFdel{: iKUN gains $+7.08\pm0.65$, }\DIFdelend \DIFaddbegin \DIFadd{, from $+9.44\pm0.92$ on iKUN down to $+0.43\pm0.19$ on }\DIFaddend FlexHook V1\DIFdelbegin \DIFdel{$+0.67\pm0.13$, and FlexHook V2 $+0.18\pm0.06$}\DIFdelend \DIFaddbegin \DIFadd{. The gain decreases monotonically as the host's baseline moving-class HOTA increases, across three different architectures: the weaker the host's own motion understanding, the more the compensated motion cues contribute}\DIFaddend .
