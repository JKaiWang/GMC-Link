...
\begin{table}[t]
  \centering
  \caption{Pooled HOTA on the Refer-KITTI test sets (mean $\pm$ std over three seeds). The DetA and AssA cells give the reproduced host value and the value with the GMC module. $^\ddagger$ denotes our reproduction using the official implementation, under the host's own frame convention (Sec.~\ref{sec:setup}).}
  \label{tab:main}
  \footnotesize
  ...
  \begin{tabular}{lcccc}
    ...
    TransRMOT~\cite{transrmot}  & 45.757$^\ddagger$ & 46.149 $\pm$ 0.025 & 36.64${\to}$36.95 & 57.40${\to}$57.89 \\
