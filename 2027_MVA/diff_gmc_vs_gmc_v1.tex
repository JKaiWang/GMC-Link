...
%DIF ADD gmc_v1.tex   Sat Aug 22 18:05:41 2026
...
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=12cm]{figures/Architecture.png}
  \DIFaddendFL \caption{The host model, iKUN in this case, provides tracking scores. The \DIFdelbeginFL \DIFdelFL{GMC-Link }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{GMC Module }\DIFaddendFL estimates camera motion, derives residual motion, aligns it with language, and provides an extra cue to adjust the tracking scores.}
...
\section{Method}
\label{sec:method}
Figure~\ref{fig:architecture} shows the proposed RMOT framework with the global motion compensation (GMC) \DIFdelbegin \DIFdel{module}\DIFdelend \DIFaddbegin \DIFadd{Module}\DIFaddend . The GMC \DIFdelbegin \DIFdel{module }\DIFdelend \DIFaddbegin \DIFadd{Module }\DIFaddend is attached to a two-stage RMOT method, iKUN~\cite{ikun} in this case. iKUN performs object detection and tracking and then outputs appearance-language matching scores. The inputs to the GMC \DIFdelbegin \DIFdel{module }\DIFdelend \DIFaddbegin \DIFadd{Module }\DIFaddend are the sequence of object bounding boxes across frames, the video frames, and the language expression. Based on these inputs, the GMC \DIFdelbegin \DIFdel{module }\DIFdelend \DIFaddbegin \DIFadd{Module }\DIFaddend estimates camera motion, extracts residual object motion, aligns it with the language expression, and produces a matching score. The score is then combined with the host model's predicted score, as an embodiment of the late fusion scheme. The \DIFdelbegin \DIFdel{complete pipeline of the GMC module is illustrated in Figure~\ref{fig:pipeline}, which }\DIFdelend \DIFaddbegin \DIFadd{module }\DIFaddend consists of four stages: ego-motion compensation, multi-scale residual velocity extraction, motion-language alignment, and late fusion.
...
\DIFdelendFL \DIFaddbeginFL \setlength{\tabcolsep}{2pt}
  \begin{tabular}{lccc}
    \hline \DIFaddendFL 
    Host                        & Published & \DIFdelbeginFL \DIFdelFL{Reproduced }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{GMC Module           }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{w/ GMC module }%DIFDELCMD < & %%%
\DIFdelendFL Gain                      \\
    \hline
...
In future work, we plan to investigate more robust camera motion estimation methods and richer motion features that can better handle complex dynamic scenes. We also hope to evaluate \DIFdelbegin \DIFdel{GMC-Link }\DIFdelend \DIFaddbegin \DIFadd{the GMC Module }\DIFaddend on a broader range of referring multi-object tracking frameworks to further improve its generalization.
