% SUPERSEDED 2026-08-19 — the live working file is now 2027_ICASSP/gmc_v1.tex.
% Keep this one for comparison only; do not edit. Content corrections to the live
% file are tracked in issues #23 (numeric/factual) and #24 (narrative).
%
...
%  \item We show that among the module's design choices, camera motion compensation is the key factor: it is the only component whose removal significantly degrades both moving-class ($p{=}0.006$) and static-class ($p{<}10^{-4}$) HOTA.
  \item Based on iKUN, the proposed plug-in raises moving-class HOTA by $4.69$. The performance gains can be observed in various settings.
\end{itemize}
